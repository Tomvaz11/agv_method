# AGV Prompt Template: Tocrisna v2.3 - Definição da Arquitetura Técnica
...
11. **Configuração do Projeto (`pyproject.toml`):** A arquitetura não se limita ao código-fonte, mas também à configuração que o suporta. **Você DEVE gerar o conteúdo inicial completo e detalhado para um arquivo `pyproject.toml`** na raiz do projeto. Este arquivo é crucial e deve:
    *   Definir os metadados básicos do projeto (nome, versão, descrição, autores, etc.).
    *   Listar as dependências essenciais de **produção** na seção `[project.dependencies]`, incluindo versões específicas se souber as recomendadas.
    *   Listar as dependências de **desenvolvimento** (como `pytest`, `pytest-cov`, `pyright`, `ruff`) na seção `[project.optional-dependencies]dev`.
    *   Incluir a seção `[project.scripts]` se houver um ponto de entrada principal.
    *   **CRUCIAL:** Incluir as seções `[tool.pytest.ini_options]` e `[tool.pyright]` (ou `[tool.pylance]`) para configurar explicitamente o `pythonpath` e `extraPaths` para apontar para o diretório `src/`, garantindo a consistência entre o ambiente de execução de testes e a análise estática do IDE.
